	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'servants-of-the-scourge':
				dialogue = CoolUtil.coolTextFile(Paths.txt('servants-of-the-scourge/servants'));
			case 'scourge-of-the-universe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('scourge-of-the-universe/universe'));
			case 'universal-collapse':
				dialogue = CoolUtil.coolTextFile(Paths.txt('universal-collapse/collapse'));
			...
			case 'terraria':
{
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('terraria/sky'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);

					var city:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('terraria/city'));
					city.scrollFactor.set(0.3, 0.3);
					city.setGraphicSize(Std.int(city.width * 0.85));
					city.updateHitbox();
					add(city);
					...
					for (i in 0...5)
					{
							var light:FlxSprite = new FlxSprite(city.x).loadGraphic(Paths.image('terraria/win' + i));
							light.scrollFactor.set(0.3, 0.3);
							light.visible = false;
							light.setGraphicSize(Std.int(light.width * 0.85));
							light.updateHitbox();
					...
					var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('terraria/behindTrain'));
					add(streetBehind);

					phillyTrain = new FlxSprite(2000, 360).loadGraphic(Paths.image('terraria/train'));
					if(FlxG.save.data.distractions){
						add(phillyTrain);
					}
					...
					var street:FlxSprite = new FlxSprite(-40, streetBehind.y).loadGraphic(Paths.image('terraria/street'));
					add(street);
			}
			case 'stage':
				{
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'Scourge-Of-The-Universe') 
		{	
			switch (curBeat)
			{
				case 104:
					remove(dad);
					dad = new Character(100, 100, 'angry-devourer');
					add(dad);
					dad.x += 0;
					dad.y += 300;
				case 168:
					remove(dad);
  					dad = new Character(100, 100, 'devourerofgods');
					add(dad);
					dad.x += 0;
					dad.y += 300;
				case 232:
